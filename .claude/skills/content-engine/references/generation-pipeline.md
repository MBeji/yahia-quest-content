...
| SQL/JSON exporter     | **deterministic script, never an LLM**: `scripts/content/build.ts` (Zod-validated files → one idempotent UUIDv5 `sql/content/<subject>.sql`, emitted by `apply-content.yml`) — hand-written SQL is forbidden                 |
...
5. **Parent-authored content is never touched** by the generator (only admin/`content/` rows are
   upserted and pruned).
6. **Nothing but `content/` files in a commit** (see below) — the SQL is emitted at apply time,
   never committed, never a migration.

## The reproducible files → gates → PR → apply procedure (read it — the old one is a trap)

Since étude 24 (D-3) content **no longer travels as migrations**. You write files; the gates judge
them; the corpus PR merges them; `apply-content.yml` compiles each subject to one stable
`sql/content/<subject>.sql` and applies it. The sequence for a change, from the engine clone with
the corpus linked (method, Phase 0.1):
...
2. **Measure the tranche before committing:** `npm run content:tranche -- --changed` — key strictly
   the longest option (aim 0), near-duplicate pairs vs the subject's **published** chapters, and
   **template candidates** (same task frame across ≥ 2 chapters, décor swapped). Fix the first two;
   put the template groups in the auditor's mandate (method B3, point 2).
   **One command** replays the seven Content CI stages in CI order plus these measures, keeps going
   after a red and prints one verdict: `npm run content:gates -- --tranche`.
3. **Commit the `content/` files only** — plus `content/CATALOGUE.md`, regenerated by
   `content:catalogue` (which `content:gates` runs), whenever a chapter or subject is new: the CI
   compares it by `git diff`.
   - ⛔ **The #1 pipeline trap is now `content:build` itself.** Without `--sql-dir` it writes
     timestamped migrations into the **public engine** repo — a dead channel, and a corpus leak that
     `leak:check` blocks. Never run it. To eyeball the SQL (optional, never committed):
     `node --experimental-strip-types scripts/content/build.ts --subject <id> --sql-dir /tmp/sql-check`.
   - A shared lycée directory (`compileTo` — étude 16: tronc commun authored once, compiled to one
     subject per section — `content/README.md` §Mutualisation) is still one directory of files. Never
...
4. **Stop and report** (files, counts, gate summary, tranche measures). Push/PR only when asked.
   **Campaign / multi-chapter work**: deliver in slices of **≤4 complete chapters** — local commit per
   chapter, one PR per slice, never a whole subject held unpushed (a cut session loses everything) —
   per the méthode (`FableEtudes/METHODE-GENERATION-CONTENU.md`, rule T-10).
5. **Publish after the merge — a merge applies nothing.** `apply-content.yml` is `workflow_dispatch`
   only: `gh workflow run apply-content.yml -f subjects="<id>" -f dry_run=true`, then
   `dry_run=false`; check the run and its `content_releases` row (method § B3).
