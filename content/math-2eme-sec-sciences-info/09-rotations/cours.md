...
_Exemple détaillé_ : cherchons tous les réels α de ]0 ; π[ tels que le carré de centre O soit globalement invariant par la rotation directe de centre O et d'angle α. Le quart de tour direct fait glisser chaque sommet sur le suivant : le carré retombe sur lui-même, donc α = π/2 convient. Le demi-tour échange chaque sommet avec le sommet opposé : il convient aussi, donc α = π. Aucune autre valeur ne marche, car un sommet doit se poser sur un sommet, et les sommets ne sont espacés que de quarts de tour. Les valeurs cherchées sont donc **π/2 et π**.
