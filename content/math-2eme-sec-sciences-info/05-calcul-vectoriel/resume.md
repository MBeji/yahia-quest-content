...
- **Repère cartésien** (O, i⃗, j⃗) : OM⃗ = xi⃗ + yj⃗ définit les coordonnées M(x, y) ; O est l'origine, (O, i⃗) l'axe des abscisses, (O, j⃗) l'axe des ordonnées. **AB⃗(xB − xA ; yB − yA)** (arrivée moins départ). ABCD est un parallélogramme ⟺ AB⃗ = DC⃗ — c'est ainsi qu'on trouve un quatrième sommet.
...
- **Les trois formules du repère orthonormé** : ‖u⃗(x ; y)‖ = √(x² + y²) ; AB = √((xB − xA)² + (yB − yA)²) ; u⃗ ⊥ v⃗ ⟺ xx' + yy' = 0. Ne pas confondre avec la colinéarité xy' − x'y = 0.
...
- **Thalès vectoriel** : si M ∈ (AB), N ∈ (AC) et (MN) ∥ (BC), alors AM⃗ = xAB⃗ entraîne AN⃗ = xAC⃗ et MN⃗ = xBC⃗. Version « trois parallèles » (**théorème de la projection**) : si (AA'), (BB') et (CC') sont trois droites parallèles, alors AB⃗ = xAC⃗ entraîne A'B'⃗ = xA'C'⃗.
