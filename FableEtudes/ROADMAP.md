...
- [ ] **P6. é35 — comprendre la théorie : le patron de notion en sept temps, pilote maths 9ᵉ.**
      ⛔ **Statut `brouillon`** (écrite le 2026-09-16) — rien n'est exécutable avant la validation
      de la fiche et l'arbitrage de **Q-1** (le seul lot moteur : un bloc `verifie` à réponse
      repliée, deux balises inertes, six contrôles structurels dans `content:qa`). Le constat qui
      la fonde, mesuré sur `main` : l'appareil de blocs de é18 est **presque inutilisé pour le
...
| **Brouillon écrit, non arbitré**      | Q-1…Q-6 à arbitrer (Q-1 : le lot moteur) | é35 (comprendre la théorie — patron de notion en sept temps, pilote maths 9ᵉ) — approfondissement V1, ligne **P6** du §3 ; aucune ouverture, rien à dégeler : à **valider** |
