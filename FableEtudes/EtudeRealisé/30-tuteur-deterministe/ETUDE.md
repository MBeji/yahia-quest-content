...
ALTER TABLE public.competencies
  ...
  ADD COLUMN IF NOT EXISTS p_transit NUMERIC NOT NULL DEFAULT 0.15 CHECK (p_transit BETWEEN 0.02 AND 0.18);
```
...
> **au-dessus** du seuil : sur cette compétence une lacune deviendrait **indétectable**, quel que
> soit le nombre d'erreurs — et avec elle s'éteindraient le marquage `suspect` (R-8) et le
> rebranchement « cause racine » du lot 4.
>
> **ARBITRÉ le 2026-09-01 (privé#247 item 2) : la borne haute passe de 0,40 à 0,18**, valeur que
> le CHECK ci-dessus porte désormais — migration moteur `20260901200000_p_transit_bound_keeps_gap_detectable`.
> À 0,18 le plancher vaut ≈ **0,202**, sous le seuil de R-5 avec ≈ 0,05 de garde.
>
> Ce que l'arbitrage a écarté, et pourquoi : **documenter seulement** — c'est ce que faisait déjà
> ce paragraphe, et un paragraphe ne refuse rien ; **rendre R-5 relatif au plancher** — le plus
> propre conceptuellement, mais il touche R-5, R-8 et le rebranchement du lot 4 avec leurs deux
> suites, et rien ne casse aujourd'hui qui le justifie. Coût du resserrement : **nul** — aucune
> compétence du corpus n'écrit `pTransit` (registre `math`, 62 entrées, re-vérifié ce jour), donc
> toutes les lignes tiennent le défaut 0,15.
>
> La solidarité des deux nombres — le CHECK et le seuil de R-5 — n'est plus une remarque : elle
> est **épinglée des deux côtés**, par `supabase/tests/75_adaptive_belief.test.sql` (0,30 refusé,
> 0,18 accepté) et par `scripts/adaptive/__tests__/belief-model.test.mjs` (au maximum, le plancher
> reste sous le seuil). Un élargissement futur du CHECK fera **rougir** les deux.
...
| 2026-09-01 | **§3.8c** | arena#943 | **`p_transit` ne peut plus éteindre la détection de lacune — la borne haute passe de 0,40 à 0,18** (privé#247 item 2, arbitré par Mohamed ce jour). Le plancher de la croyance vaut ≈ 1,12·p(T) : au-delà de p(T) ≈ 0,22 il couvre le seuil de R-5 (0,25) et une lacune devient indétectable, quel que soit le nombre d'erreurs — avec elle s'éteignent R-8 et le rebranchement « cause racine » du lot 4. §3.8c documentait le danger depuis le lot 1 ; **un paragraphe ne refuse rien**. À 0,18 le plancher vaut ≈ 0,202, sous le seuil avec ≈ 0,05 de garde. **Coût : nul** — aucune compétence du corpus n'écrit `pTransit` (registre `math`, 62 entrées, re-vérifié ce jour), toutes tiennent le défaut 0,15. Écartés : documenter seul (déjà fait, sans effet opposable) et rendre R-5 relatif au plancher (le plus propre, le plus coûteux, rien ne casse qui le justifie). La solidarité des deux nombres est désormais **épinglée des deux côtés** : pgTAP (0,30 refusé, 0,18 accepté, plan 27 → 29) et `belief-model.test.mjs` (au maximum, le plancher reste sous le seuil). Un élargissement futur du CHECK fera rougir les deux. |
---
