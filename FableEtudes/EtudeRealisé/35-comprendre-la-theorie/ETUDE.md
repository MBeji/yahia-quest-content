...
| **8** | **Bilan & extension** (privé) : rapport du pilote, KPI §1.4, verdict Q-5, étude → `livrée`                                                                                                                               | `ETUDE.md` §8, `README.md` (index), STATUS.md (moteur : §4, §6 et la ligne « cours »)                                                                                                                                                                                                      | `etudes:check`, `roadmap-sync`                                                                                                | 7 (ou 6 si Q-6 = non)   |
...
- [x] **Lots 3 à 6 — La campagne pilote, quatre tranches.** ✅ 2026-09-16 (§8)
      ...
      **conservées** (une figure vraie ne se redessine pas ; elle reçoit sa phrase de pont R-11) ;
      ...
      _Avancement_ : lots 3 à 6 ✅ — **les 20 chapitres de maths 9ᵉ sont au patron**, et
      `coursePattern: "notion"` est posé : zéro erreur en régime `error`. **Huit** chapitres
      ...
      au NOMBRE de notions (~8). **Traité le 2026-09-17** : les deux chapitres les plus larges
      ont été **regroupés, pas scindés** — `07-statistiques` 15 → 10 notions et
      `18-quadrilateres` 12 → 9 ; l'arbitrage a vu que les notions surnuméraires venaient de
...
| 2026-09-16 | 4    | privé#407  | **Lot 4 livré — la tranche algébrique.** Quatre chapitres réécrits (`19-valeur-absolue` 119 → 236 lignes, `03-calcul-litteral` 135 → 390, `04-equations-inequations` 166 → 351, `05-systemes` 139 → 281 — **chiffres mesurés sur les fichiers livrés** : une première version du journal donnait 208/346/305/246, relevés AVANT la passe Prettier qui ajoute les lignes vides protégeant les clôtures de directive) : 28 contrôles `verifie`, 38 exemples résolus, 30 pièges, 23 `coursePitfalls` déclarés, les 5 figures conservées **octet pour octet** (vérifié par extraction et comparaison des blocs `<svg>`). Ancrages du manuel : le nombre deviné du **نشاط 2 p.95** (« j'ai retranché 5/2, multiplié par 6, ajouté 75, trouvé 216 ») ouvre la traduction des problèmes, et l'aire calculée **de deux façons** des نشاط 2 p.79 / نشاط 6 p.83 fait naître les trois identités. Les deux encadrés VERBATIM du manuel (p.95 pour l'équation, p.103 pour l'inéquation) remplacent les définitions maison. **Preuve** : 143 calculs re-dérivés indépendamment, zéro faux ; gates verts (0 erreur sur 999 chapitres) ; et les 28 blocs `verifie` passés dans le **vrai moteur de rendu** — question et réponse repliée émises pour chacun, libellé arabe présent, aucun `open`, aucune figure échappée. **Trois constats remontés, aucun corrigé unilatéralement** : (a) `05-systemes` est **hors programme 9ᵉ** — le registre de transcription l'établit sur les 221 pages (`جملة` et `بمجهولين` absents, les trois encadrés du ch.7 disent « ذات مجهول واحد ») — donc ancré au quotidien, faute d'activité à moissonner ; (b) `03` (390) et `04` (351) **dépassent R-13** — et le relevé complet fait après coup montre que **8 des 10 chapitres au patron** passent 240 lignes, la maquette `06` (273) comprise : ce n'est pas une série d'exceptions, c'est la règle qui ne décrit pas ce que le patron produit ; (c) la section des trois identités remarquables a dû être **scindée en trois sections `##`** — C-6 l'a refusée à 82 lignes, et le gate avait raison : elle portait trois notions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-09-16 | 5    | privé#411  | **Lot 5 livré — la tranche géométrie A, et deux règles de mesure corrigées.** Quatre chapitres réécrits (`08-thales` 184 → 348 lignes, `09-triangle-rectangle-trigo` 168 → 443, `10-angles-cercle` 178 → 340, `11-vecteurs-translation` 167 → 335) : 31 sections, 30 contrôles `verifie`, 36 exemples résolus, 27 pièges, 30 blocs « à retenir », 18 `coursePitfalls`, et les **12 figures conservées octet pour octet**. Ancrages du manuel là où il en a : Thalès naît de l'**argument d'aires** du نشاط 1/2 p.148 (deux triangles à hauteur commune, donc des aires proportionnelles aux bases) et cite l'encadré VERBATIM p.150 ; Pythagore naît du **puzzle de découpage** du نشاط 1 p.172 et de l'aire calculée de deux façons du نشاط 2 p.173, et cite l'encadré p.173. Les deux autres chapitres sont ancrés au quotidien, faute d'activité à moissonner. **Preuve** : 111 calculs re-dérivés indépendamment, zéro faux — les vecteurs vérifiés numériquement sur des points réels du plan, Chasles et la règle du parallélogramme comprises. **Deux mesures corrigées, et c'est le vrai apport du lot** : (a) **R-13 perd son plafond par cours** (voir ci-dessus) ; (b) **C-6 comptait la mise en forme** — lignes vides et corps de `<svg>` compris. Les SEPT sections qu'il dénonçait en maths étaient sous le seuil une fois la mesure honnête, et la pire n'était pas proche : « شبه المنحرف » de `18-quadrilateres` signalée à **72 lignes** en porte **19**. Corpus entier : **336 → 262 avertissements, soit 74 faux supprimés** ; maths 9ᵉ : 8 → 1. Le seuil de 60 n'a pas bougé. **Écart assumé** : la correction de C-6 a fait dépasser `max-lines` à `qa-checks.ts` (928 lignes de code pour 750), d'où une **scission** en `qa-lesson-checks.ts` — la couture suit la matière (questions d'un côté, textes de l'autre), pas la taille. Et un piège ESM retrouvé au passage : un import sans extension `.ts` typecheck sans broncher et casse à l'exécution ; seuls trois tests lançant le gate en sous-processus l'ont vu.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-09-16 | 6    | privé#—    | **Lot 6 livré — la campagne est finie, et le drapeau est posé.** Six chapitres réécrits (`07-statistiques` 260 → 672 lignes, `18-quadrilateres` 330 → 647, `12-repere-plan` 220 → 405, `20-orthogonalite-espace` 168 → 401, `13-geometrie-espace` 201 → 380, `14-annales-sujets-types` 69 → 217), six `resume.md` réécrits **en miroir**, 33 `coursePitfalls`, et les 27 figures conservées octet pour octet. **`coursePattern: "notion"` est posé sur `content/math/subject.json`** : les six contrôles passent en régime `error` et **les 20 chapitres sortent à ZÉRO erreur**. C'est le critère d'acceptation du lot, atteint sans exception ni dérogation. Corpus maths 9ᵉ au total : **155 contrôles `verifie`, 115 `coursePitfalls`**. **Preuve** : 147 calculs re-dérivés indépendamment ; le seul écart signalé venait de mon propre vérificateur (il calculait (4√2)² en virgule flottante et trouvait 32,000000000000014) — le cours disait vrai, comme au lot 3. Rendu vérifié dans `renderLesson` sur les six chapitres. **Décision prise plutôt qu'une dérogation** : `14-annales-sujets-types` est un chapitre de **méthodologie**, et la recommandation était de l'exempter par un champ de schéma. Constat en le relisant : tout y est déjà de la méthode, et une stratégie sans application résolue n'est qu'un conseil. Il est donc passé au patron comme les autres — chaque stratégie a reçu son exemple chiffré et son contrôle — et **aucune échappatoire n'a été ajoutée au moteur** : un champ d'exemption aurait été le premier trou par où le patron aurait fui. **Reste ouvert** : `07-statistiques` porte 15 notions et `18-quadrilateres` 12, au-dessus du seuil de ~8 de R-13 amendée — candidats à la scission, qui est une question de programme. Et `14-annales` reste sans `domain` : il ne relève d'aucun, et le corriger changerait l'affichage du hub, hors périmètre d'un lot de patron.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-09-17 | —    | privé#—    | **Arbitrage du propriétaire, et la passe qui en découle : la glose française.** « Tu peux laisser du français si déjà le terme arabe existe, car en lycée on bascule en français. » Le constat est juste et il change une règle : l'élève qui apprend الوتر en 9ᵉ rencontre _hypoténuse_ un an plus tard. **R-14 est amendée** — chaque terme-clé porte son équivalent français entre parenthèses à sa **première apparition** dans le chapitre, et nulle part ailleurs ; jamais dans une formule, un tableau ou une légende (la parenthèse y casse le sens de lecture RTL) ; l'arabe garde la tête, le français est une étiquette. **Passe faite** : 26 → **98 gloses** sur les 20 chapitres, posées dans les blocs de savoir. Ce qui reste interdit est ce que la règle n'est pas : une **phrase** à moitié française au milieu d'une proposition arabe — c'est cela, et cela seul, que j'avais purgé aux lots 5 et 6. **Contrôle de placement** : sept gloses étaient tombées dans une épigraphe ou dans la **réponse** d'un `::: verifie` — l'élève y aurait rencontré le terme français dans la correction avant la leçon. Déplacées dans leur bloc de savoir. Rendu vérifié sur les 20 chapitres : aucune glose n'atterrit dans une formule. ⚠️ **Le piège Prettier a un second visage, découvert ici** : il ne se contente pas d'indenter les clôtures, il **réécrit à l'intérieur d'un `<svg>`** (une ligne vide après la balise ouvrante de la figure de `06-fonctions`). Inerte en SVG, mais cela casse l'invariant « octet pour octet ». Le fichier **échouait déjà à Prettier sur `main`** depuis le lot 2 : ma passe n'a pas abîmé la figure, elle a absorbé une dette latente. D'où la règle ajoutée : `prettier --check` AVANT de toucher un chapitre, et vérifier les `<svg>` par extraction plutôt que par confiance. **28 figures sur 29 sont octet pour octet** ; la 29ᵉ a gagné un saut de ligne inerte.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-09-17 | —    | privé#—    | **Scission des deux chapitres trop larges — arbitrage du propriétaire, regroupement plutôt que découpage.** Les deux candidats laissés ouverts au lot 6 sont traités : `07-statistiques` **15 → 10 notions** (672 → 645 lignes) et `18-quadrilateres` **12 → 9** (647 → 630). Le propriétaire a tranché **regrouper** dans les deux cas, contre la scission en deux chapitres : les notions surnuméraires n'étaient pas un programme trop large, c'était **ma réécriture qui avait éclaté des sections qui n'en formaient qu'une** — les trois outils de démonstration du 18 étaient une seule section avant le lot 6, et le cours les annonçait déjà comme « les trois outils ». Fusions faites avec une **phrase de pont** à chaque couture (jamais deux blocs recollés bout à bout) : au 18, les projections rejoignent le trapèze dont elles sont une propriété, et les trois outils redeviennent `## 🧰 أدوات البرهان الثلاث`. **Les 11 figures sont octet pour octet**, 0 erreur sur les 999 chapitres. ⚠️ **Un avertissement C-6 est assumé, pas contourné** : la section des trois outils fait **66 lignes de contenu** pour un seuil de 60, et le gate dit vrai — elle porte trois règles. Le seuil n'a **pas** été relevé et aucune exemption n'a été ajoutée : C-6 sort en `warn` **même sous le régime `error`** (le niveau est codé en dur dans `qa-lesson-checks.ts`), précisément pour qu'un arbitrage humain puisse le porter en le laissant visible. Trancher autrement aurait demandé de supprimer un exemple résolu — payer de l'enseignement pour satisfaire une mesure. À titre de repère, `07` après ses fusions tient à **59 lignes** sur sa plus longue section, une phrase sous le même seuil. **Contrôle de langue** : le détecteur de _phrase_ latine en contexte arabe (≥ 2 mots latins consécutifs hors parenthèses, formule, code et `<svg>`) passe les 40 fichiers du chapitre à **zéro** — la glose entre parenthèses de R-14 reste, la phrase à moitié française a disparu.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
